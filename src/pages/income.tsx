...
import { Active, Passive } from '../icons';
...
export default function Income(): JSX.Element {
  ...
  const filteredData = selectedAttribute
    ? user.march23.Received.filter((item) => item.attribute === selectedAttribute)
    : user.march23.Received;
    ...
    <Layout>
      <div className="p-6 flex flex-col pb-28">
        ...
        <div className="pt-10 flex flex-wrap justify-evenly gap-10">
          <CircularButton
            id={1}
            selected={selectedAttribute === 'active'}
            option="active"
            icon={<Active />}
          ...
          <CircularButton
            id={2}
            selected={selectedAttribute === 'passive'}
            option="passive"
            icon={<Passive />}
            onButtonClick={onButtonClick}
          />
